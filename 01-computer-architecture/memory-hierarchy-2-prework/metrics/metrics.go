...
func AverageAge(user_ages UserAges) float64 {
	var total uint32
	for _, age := range user_ages {
		total += age
	}
	return float64(total) / float64(len(user_ages))
}
...
func LoadData() (UserMap, UserAges) {
	...
	for _, line := range userLines {
		...
		userAges = append(userAges, uint32(age))
		address := line[3]
